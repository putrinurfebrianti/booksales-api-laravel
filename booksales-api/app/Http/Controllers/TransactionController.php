    ...
    public function show(Transaction $transaction)
{
    ...
    // cek apakah transaksi milik user login
    if ($transaction->customer_id !== $user->id) {
        return response()->json([
            'success' => false,
            'message' => 'Access denied! You are not the owner of this transaction.'
        ], 403);
    }
...
public function update(Request $request, Transaction $transaction)
{
    ...
    // Hitung selisih quantity baru dengan lama
    $oldQuantity = $transaction->total_amount / $book->price;
    $diffQuantity = $request->quantity - $oldQuantity;

    // Cek stok jika quantity bertambah
    if ($diffQuantity > 0 && $book->stock < $diffQuantity) {
        return response()->json([
            'success' => false,
            'message' => 'Stok barang tidak cukup!'
        ], 400);
